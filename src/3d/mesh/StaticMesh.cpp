...
std::unique_ptr<StaticMesh> StaticMesh::FromOBJ(std::ifstream& file) {
    auto result = std::make_unique<StaticMesh>();
        ...
        if (line.starts_with("v ")) {
            ...
            if (line.find('/') != std::string::npos) {
                ...
                sscanf(line.c_str(), "f %d/%d/%d %d/%d/%d %d/%d/%d", &p1, &t1, &n1,
                                                                     &p2, &t2, &n2,
                                                                     &p3, &t3, &n3);
                result->faces_.push_back(int3(p1, t1, n1));
                result->faces_.push_back(int3(p2, t2, n2));
                result->faces_.push_back(int3(p3, t3, n3));
            } else {
                ...
                result->faces_.push_back(int3(p1, p2, p3));
            }
        }
    }
